...
import type { StrokeRecord, Utterance } from "../../src/contracts-capture";
import assignUtteranceDefault, { assignUtterance } from "../../src/assign";
...
describe("assignUtterance — the two rhythms the founder actually draws in", () => {
  it("draw then speak: each utterance lands in the shape that was drawn just before it", () => {
    ...
    expect(assignUtterance(U1, strokes, LATER)).toEqual({ strokeId: "S1", final: true });
    expect(assignUtterance(U2, strokes, LATER)).toEqual({ strokeId: "S2", final: true });
    expect(assignUtterance(U3, strokes, LATER)).toEqual({ strokeId: "S3", final: true });
  });
  ...
  it("speak then draw: the label spoken up to a pre-roll before the stroke still lands in it", () => {
    ...
    const strokes = [stroke("S1", 1200), stroke("S2", 4800), stroke("S3", 8800)];
    expect(assignUtterance(U1, strokes, LATER)).toEqual({ strokeId: "S1", final: true });
    expect(assignUtterance(U2, strokes, LATER)).toEqual({ strokeId: "S2", final: true });
    expect(assignUtterance(U3, strokes, LATER)).toEqual({ strokeId: "S3", final: true });
  });
...
describe("assignUtterance — when nobody can claim the speech", () => {
  it("speech starting 2 s before the only stroke is an orphan, but not until the pre-roll has elapsed", () => {
    ...
    expect(assignUtterance(u, strokes, 1000)).toEqual({ strokeId: null, final: false });
    // At the pre-roll boundary the window is closed: S1's pointer-down at 2500 is too late, forever.
    expect(assignUtterance(u, strokes, 1500)).toEqual({ strokeId: null, final: true });
    expect(assignUtterance(u, strokes, LATER)).toEqual({ strokeId: null, final: true });
  });
  ...
  it("a long utterance whose stroke arrives 2.9 s later is an orphan, decided at onset + pre-roll", () => {
    ...
    expect(assignUtterance(u, strokes, 1599)).toEqual({ strokeId: null, final: false });
    expect(assignUtterance(u, strokes, 1600)).toEqual({ strokeId: null, final: true });
  });

  it("with no strokes at all there is nothing to claim it", () => {
    expect(assignUtterance(U1, [], LATER)).toEqual({ strokeId: null, final: true });
  });
  ...
  it("a palm tap cannot capture speech: it never became a stroke, so it is not in the list", () => {
    ...
    expect(assignUtterance(utterance(1, 2000, 4000), strokes, LATER)).toEqual({ strokeId: "S1", final: true });
  });
});
...
describe("assignUtterance — the documented consequence of 'latest candidate wins'", () => {
  it("speech that begins within the pre-roll BEFORE the next stroke belongs to the NEXT stroke", () => {
    ...
    expect(assignUtterance(utterance(1, 2000, 5000), strokes, LATER)).toEqual({ strokeId: "S2", final: true });
    // Just outside the window, the same gesture stays with S1.
    expect(assignUtterance(utterance(2, 1499, 5000), strokes, LATER)).toEqual({ strokeId: "S1", final: true });
  });
  ...
  it("a stroke exactly at onset + pre-roll is still a candidate (the boundary is inclusive)", () => {
    ...
    expect(assignUtterance(utterance(1, 0, 800), strokes, LATER)).toEqual({ strokeId: "S2", final: true });
  });
});
...
describe("assignUtterance — shape of the function", () => {
  it("does not care about the order strokes are given in", () => {
    ...
    for (const u of [U1, U2, U3]) {
      expect(assignUtterance(u, shuffled, LATER)).toEqual(assignUtterance(u, sorted, LATER));
    }
  });

  it("breaks a pointer-down tie in favour of the stroke recorded last", () => {
    const strokes = [stroke("A", 1000), stroke("B", 1000)];
    expect(assignUtterance(utterance(1, 1000, 2000), strokes, LATER).strokeId).toBe("B");
  });

  it("honours a custom pre-roll", () => {
    const strokes = [stroke("S1", 0), stroke("S2", 3000)];
    const u = utterance(1, 2000, 5000);
    expect(assignUtterance(u, strokes, LATER, { preRollMs: 0 })).toEqual({ strokeId: "S1", final: true });
    expect(assignUtterance(u, strokes, 2500, { preRollMs: 1000 })).toEqual({ strokeId: "S2", final: false });
  });
