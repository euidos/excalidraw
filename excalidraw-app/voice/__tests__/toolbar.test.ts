...
import { GLYPH_FULL_SCALE, glyphLevel, METER_FULL_SCALE } from "../level";
import { buttonTitle, buttonVisualState } from "../toolbar";
...
describe("buttonVisualState — the mic glyph says whether the microphone is hearing anything", () => {
  ...
  it("ignores a stale level once idle (status.level keeps the last RMS after a disarm)", () => {
    const v = buttonVisualState(
      base({ mode: "idle", level: GLYPH_FULL_SCALE, speaking: true }),
    );
    expect(v.level).toBe(0);
  ...
  it("fills through level.ts's GLYPH mapping, not the VAD meter's axis", () => {
    for (const rms of [0.01, 0.02, 0.08, 0.2]) {
      expect(
        buttonVisualState(base({ mode: "latched", level: rms })).level,
      ).toBeCloseTo(glyphLevel(rms), 10);
    }
    expect(
      buttonVisualState(base({ mode: "holding", level: GLYPH_FULL_SCALE }))
        .level,
    ).toBe(1);
    // The round-4b defect: on the meter's axis normal speech was already at 1.00 and the capsule stopped moving.
    expect(
      buttonVisualState(base({ mode: "latched", level: METER_FULL_SCALE }))
        .level,
    ).toBeLessThan(1);
  });

  it("clamps a level louder than the glyph's full scale instead of overflowing the capsule", () => {
    expect(buttonVisualState(base({ mode: "latched", level: 1 })).level).toBe(
      1,
    );
  });

  it("turns the accent on only while an utterance is OPEN — loud room noise is not 'recognized'", () => {
    const loud = buttonVisualState(
      base({ mode: "latched", recording: true, level: METER_FULL_SCALE }),
    );
  ...
  it("keeps recording and mic-missing as their own signals", () => {
    expect(
      buttonVisualState(base({ mode: "latched", recording: true })).classes[
        "voice-tool--recording"
      ],
    ).toBe(true);
    for (const mic of ["denied", "missing", "error"] as const) {
      expect(
        buttonVisualState(base({ mic })).classes["voice-tool--mic-missing"],
      ).toBe(true);
    }
...
describe("buttonTitle — the toolbar says how many transcripts were thrown away", () => {
  ...
  it("names the count and the text that was filtered", () => {
    const title = buttonTitle(
      status({ dropped: 2, lastDropped: "Subtitles by amara.org" }),
    );
  ...
  it("keeps the failure line as well: two different channels, both readable", () => {
    const title = buttonTitle(
      status({ dropped: 1, lastError: "STT server unreachable" }),
    );
