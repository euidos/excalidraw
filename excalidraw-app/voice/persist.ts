/**
 * The load-path sweep: voice scaffolding a dead take left behind, removed before anyone sees it.
 *
 * 0.1.0's version of this module also OWNED persistence — it read and wrote the vanilla excalidraw-app
 * localStorage/IndexedDB keys, because the voice tool then lived in a thin wrapper around the published package
 * that had no storage of its own. In excalidraw-app those keys belong to `data/LocalData.ts` and the room to
 * `data/euidosStorage.ts`, so the port's `loadInitialData` / `createPersister` / `libraryAdapter` had no caller
 * and never gained one. They are gone (phase-2 RETRO G-P2.10); what is left is the pure sweep and its two
 * callers, `App.tsx initializeScene` (local scene) and `Collab.initializeRoom` (room scene).
 */
